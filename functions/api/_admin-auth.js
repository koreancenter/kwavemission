...
export function getAdminConfig(context) {
  ...
  return {
    email: String(env.ADMIN_EMAIL || LOCAL_AUTH_DEFAULTS.email).trim().toLowerCase(),
    password: String(env.ADMIN_PASSWORD || LOCAL_AUTH_DEFAULTS.password),
    secret: String(env.JWT_SECRET || LOCAL_AUTH_DEFAULTS.secret)
  };
}
