...
app.get('*', (req, res) => {
  ...
  if (req.path.startsWith('/assets/') || req.path.startsWith('/uploads/') || req.path.startsWith('/docs/')) {
    return res.status(404).type('text/plain').send('Not Found');
  }
  if (/\.(js|css|json|png|jpg|jpeg|gif|webp|svg|ico|woff|woff2|ttf|map|md)$/i.test(req.path)) {
    return res.status(404).type('text/plain').send('Not Found');
  }
  res.setHeader('Cache-Control', 'public, max-age=0, must-revalidate');
