...
const savedServerUrl = localStorage.getItem('ai_ollama_server_url') || '';
const cleanServerUrl = (savedServerUrl === 'https://mrpark-bali.taile6b19b.ts.net') ? '' : savedServerUrl;
...
const AIAssistant = {
  config: {
    ...
    ollamaServerUrl: cleanServerUrl,
  ...
  bindEvents(container) {
    ...
    const inputPc = container.querySelector('.ollamaPcUrl');
    const inputServer = container.querySelector('.ollamaServerUrl');

    inputPc?.addEventListener('input', (e) => {
    ...
    inputServer?.addEventListener('input', (e) => {
      const url = e.target.value.trim();
      this.config.ollamaServerUrl = url;
      localStorage.setItem('ai_ollama_server_url', url);
    });

    inputServer?.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        e.preventDefault();
        btnFetchServer?.click();
      }
    });
