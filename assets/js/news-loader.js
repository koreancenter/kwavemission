(function () {
    ...
    function _extractFirstImage(post) {
        if (!post) return '';
        var rawThumb = post.thumbnail_url || post.thumbnail || '';
        if (typeof rawThumb === 'string' && rawThumb.trim()) {
        ...
        // 2. Check Markdown image ![alt](url)
        var mdMatch = content.match(/!\[.*?\]\(\s*([^\s\)]+)/i);
        if (mdMatch && mdMatch[1]) {
            var mdSrc = mdMatch[1].trim().replace(/^<|>$/g, '');
            if (!/^(javascript|vbscript):/i.test(mdSrc)) {
                return mdSrc;
            }
        }
    ...
    function _normalizeDbPost(post) {
        ...
        const thumb = _extractFirstImage(post);
    ...
    async function _fetchNewsPosts() {
        try {
            ...
            const res = await fetch('/api/get-posts?type=news');
            if (res.ok) {
                const json = await res.json();
                return _unwrapItems(json).map(_normalizeDbPost);
            }
        } catch (err) {
            console.warn('Failed to fetch news posts via primary path:', err);
        }
        return [];
    }
    ...
    async function _fetchNoticePost() {
        try {
            ...
            const res = await fetch('/api/get-posts?type=notice');
            if (res.ok) {
                const json = await res.json();
                const notices = _unwrapItems(json);
                return notices.length > 0 ? _normalizeDbPost(notices[0]) : null;
            }
        } catch (err) {
            console.warn('Failed to fetch notice post via primary path:', err);
        }
        return null;
    }
    ...
    window.openNoticeBannerModal = function () {
        ...
        } else if (typeof window.openMdModal === 'function') {
            window.openMdModal('notice-main');
        }
    };
    ...
    async function loadNews() {
        ...
        var postImage = (featuredPost.thumbnail && typeof featuredPost.thumbnail === 'string') ? featuredPost.thumbnail.trim() : '';
        var displayImg = postImage || randomImg;
        var altText = postImage ? _escapeHtml(featuredPost.title) : '인도네시아의 비식별 풍경 이미지';
        ...
        container.innerHTML =
            '<article class="news-feature-card group" data-news-index="0" tabindex="0" role="button">' +
                ...
                '<div class="news-feature-inner">' +
                    '<div class="news-feature-meta">' +
                        ...
                        '<span class="text-xs text-stone-500 font-medium">' + _escapeHtml(featuredPost.date) + '</span>' +
                    '</div>' +
                    '<div>' +
                    ...
                    '<div class="news-feature-footer">' +
                        ...
                        '<span class="news-feature-publisher text-xs text-stone-500 font-medium">' + FEATURED_PUBLISHER_LABEL + '</span>' +
                    '</div>' +
                '</div>' +
            '</article>' +
            ...
            '<section class="field-voices" aria-labelledby="field-voices-title">' +
                '<div class="field-voices-heading text-center sm:text-left">' +
                    '<span class="text-xs font-semibold tracking-widest uppercase text-stone-500 block mb-1">ANONYMISED TESTIMONIES</span>' +
...
modalContent.innerHTML =
    '<div class="relative shrink-0 px-6 sm:px-8 pt-5 sm:pt-7 pb-4 sm:pb-5 border-b border-slate-100 bg-white">' +
        ...
        '</button>' +
        '<div class="flex items-center gap-2 text-xs text-stone-500 font-medium mb-1 pr-10 tracking-wider">' +
            '<span class="px-2.5 py-0.5 rounded-md bg-stone-100 text-stone-700 border border-stone-200/80 font-semibold text-[11px] sm:text-xs tracking-wider uppercase">' + _escapeHtml(category || '주간 선교 소식') + '</span>' +
            '<span class="text-stone-300">•</span>' +
            '<span class="text-stone-500 text-xs tracking-wide">' + _escapeHtml(date) + '</span>' +
        '</div>' +
    ...
    '</div>';
        ...
        try {
            var markdownReady = typeof window.loadMarked === 'function'
                ? window.loadMarked()
                : Promise.resolve(Boolean(window.marked));
            var md = '';
            if (postSource === 'db' && postId) {
                if (window.KWaveApi && typeof window.KWaveApi.fetchPostById === 'function') {
                    var dbPost = await window.KWaveApi.fetchPostById(postId);
                    md = dbPost && dbPost.content ? dbPost.content : '';
                } else {
                    var res = await fetch('/api/get-posts?id=' + encodeURIComponent(postId));
                    if (res.ok) {
                        var json = await res.json();
                        md = json && json.data && json.data.content ? json.data.content : '';
                    }
                }
            } else if (file) {
