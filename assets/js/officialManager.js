(function () {
    ...
    const html = letters.map((letter) => {
      ...
      return `
        <tr class="hover:bg-slate-50/80 transition-colors border-b border-slate-100 group" data-id="${letter.id}" data-token="${letter.secret_token}">
          <td class="p-3 text-xs text-slate-400 font-mono text-center">${letter.id}</td>
          <td class="p-3 text-xs font-bold text-slate-900 whitespace-nowrap">
            <span class="inline-flex items-center px-2 py-0.5 rounded-md bg-slate-100 text-slate-800 font-mono border border-slate-200">${safeDocNo}</span>
          </td>
          <td class="p-3 text-xs text-slate-700 font-medium max-w-[150px] truncate" title="${safeReceiver}">
            ${safeReceiver}
          </td>
          <td class="p-3">
            <div class="font-semibold text-slate-900 text-sm hover:text-amber-700 transition-colors cursor-pointer line-clamp-1 btn-view-letter" title="${safeTitle}">
              ${safeTitle}
            </div>
  ...
  async function loadOfficialList() {
    const tbody = document.getElementById('officialTableBody');
    if (tbody) {
      tbody.innerHTML = '<tr><td colspan="6" class="p-6 text-center text-slate-400">발송 공문 목록을 불러오는 중...</td></tr>';
    }
...
<p><em>※ 첨부: K-Wave Mission 2026 하반기 선교 사역 브로슈어 및 동역 신청서 1부. 끝.</em></p>`;
  ...
  function init() {
    ...
    if (editorEl && window.HtmlSourceEditor) {
      officialTiptapInstance = window.HtmlSourceEditor.create('officialTiptapEditor', 'officialTiptapToolbar', 'officialContent');
    }
    bindEvents();
  }
