(function () {
  ...
  function initTiptapEditor() {
    ...
    if (!window.HtmlSourceEditor) {
      document.addEventListener('DOMContentLoaded', initTiptapEditor, { once: true });
      return;
    }

    tiptapInstance = window.HtmlSourceEditor.create('postTiptapEditor', 'postTiptapToolbar', 'postContent');
  }
  ...
  function renderPostList() {
    ...
    const headerBadge = document.getElementById('postHeaderBadge');
    if (headerBadge) headerBadge.textContent = `${filteredPosts.length}개`;
    ...
    if (!filteredPosts.length) {
      tbody.innerHTML = '<tr><td colspan="7" class="p-8 text-center text-slate-400">검색 조건에 맞는 게시글이 없습니다.</td></tr>';
      updateBatchBar();
      return;
    }

    tbody.innerHTML = visiblePosts.map(post => {
      const plainExcerpt = stripHtmlTags(post.content || '').substring(0, 75);
      return `
        <tr class="border-b border-slate-100 hover:bg-slate-50/70 transition-colors group">
          ...
          <td class="p-3 text-center">
            <span class="type-chip ${post.type === 'news' ? 'type-news' : 'type-notice'}">${post.type === 'news' ? '뉴스' : '공지'}</span>
          </td>
          <td class="p-3">
            <div class="flex flex-col">
              <button type="button" onclick="previewPost(${post.id})" class="table-title-btn text-left font-semibold text-slate-900 hover:text-amber-700 transition-colors leading-snug text-sm cursor-pointer p-0 bg-transparent border-0 outline-none shadow-none line-clamp-1">${window.AdminUI.escapeHtml(post.title)}</button>
  ...
  async function deletePost(id) {
    const confirmed = await safeConfirm(`정말 #${id}번 게시글을 삭제하시겠습니까?`, {
      title: '게시글 삭제',
      confirmText: '삭제',
      danger: true
    });
    ...
    try {
      await window.AdminApi.api.post('/api/delete-post', formData, { isFormData: true });
      window.AdminUI.showToast('게시글이 삭제되었습니다.', 'success');
      loadPostList();
      if (window.DashboardManager && window.DashboardManager.refresh) {
        window.DashboardManager.refresh();
      }
    } catch (err) {
      window.AdminUI.showToast(err.message || '삭제 중 오류가 발생했습니다.', 'error');
    }
  }
  ...
  function bindPostEvents() {
    ...
    if (btnBatchDelete) {
      btnBatchDelete.addEventListener('click', async () => {
        ...
        const confirmed = await safeConfirm(`선택한 ${selectedIds.length}개 게시글을 영구 삭제하시겠습니까?`, {
          title: '선택 항목 삭제',
          confirmText: '일괄 삭제',
          danger: true
        });
        ...
        try {
          ...
          window.AdminUI.showToast(`${count}개 게시글이 성공적으로 삭제되었습니다.`, 'success');
          if (selectAllChk) selectAllChk.checked = false;
          loadPostList();
          if (window.DashboardManager && window.DashboardManager.refresh) {
            window.DashboardManager.refresh();
          ...
          window.AdminUI.showToast(e.message || '일괄 삭제 처리 중 문제가 발생했습니다.', 'error');
        }
      });
    }
    ...
    const batchChangeType = async (newType) => {
      const selectedIds = getSelectedPostIds();
      if (!selectedIds.length) {
        window.AdminUI.showToast('변경할 항목을 먼저 선택해주세요.', 'warning');
        return;
      }

      const typeName = newType === 'news' ? '뉴스' : '공지사항';
      const confirmed = await safeConfirm(`선택한 ${selectedIds.length}개 게시글의 분류를 '${typeName}'(으)로 변경하시겠습니까?`, {
        title: '분류 일괄 변경',
        confirmText: '변경'
      });
      if (!confirmed) return;
      ...
      for (const id of selectedIds) {
        ...
        try {
          await window.AdminApi.api.post('/api/update-post', formData, { isFormData: true });
          successCount++;
        } catch (e) {
