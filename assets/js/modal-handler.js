(function () {
    ...
    async function openMdModal(slug, customTitle, customBadge, customSubtitle) {
        ...
        try {
            ...
            const isHtmlDocument = String(markdownText || '').trim().startsWith('<');
            if (!isHtmlDocument && typeof window.loadMarked === 'function') {
    ...
    function openCinematicModal(cardId) {
        ...
        if (modal && data && typeof window.activateModal === 'function') {
            ...
            // Reset classes
            panel.className = 'relative rounded-2xl overflow-hidden shadow-2xl animate-fade-in-up';
    ...
    try {
        ...
        console.warn('Initial hash handling failed:', e);
    }

    if (Array.isArray(window._pendingMdModal)) {
        const pendingArgs = window._pendingMdModal;
        window._pendingMdModal = null;
        try {
            openMdModal.apply(null, pendingArgs);
        } catch (e) {
            console.warn('Failed to process pending modal call:', e);
        }
    }
})();