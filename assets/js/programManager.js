(function () {
  ...
  async function safeConfirm(message, options = {}) {
    try {
      if (window.AdminUI && typeof window.AdminUI.confirm === 'function') {
        return await window.AdminUI.confirm(message, options);
      }
    } catch (e) {
      console.warn('AdminUI.confirm error, fallback to confirm:', e);
    }
  ...
  function renderProgramList() {
    ...
    const headerBadge = document.getElementById('programHeaderBadge');
    if (headerBadge) headerBadge.textContent = `${filteredPrograms.length}개`;
    ...
    if (!filteredPrograms.length) {
      tbody.innerHTML = '<tr><td colspan="8" class="p-8 text-center text-slate-400">검색 조건에 맞는 프로그램이 없습니다.</td></tr>';
      updateProgramBatchBar();
      return;
    }

    tbody.innerHTML = visiblePrograms.map((program) => {
      const plainExcerpt = stripHtmlTags(program.description || '').substring(0, 75);
      return `
        <tr class="border-b border-slate-100 hover:bg-slate-50/70 transition-colors group">
          ...
          <td class="p-3 text-center text-slate-400 font-mono text-xs">${program.id}</td>
          <td class="p-3 text-center text-lg">${program.icon || '🎓'}</td>
          <td class="p-3 font-mono text-xs text-slate-600"><code>${window.AdminUI.escapeHtml(program.slug)}</code></td>
          <td class="p-3 text-center"><span class="px-2 py-0.5 text-xs font-semibold rounded-md bg-slate-100 text-slate-700 border border-slate-200">${window.AdminUI.escapeHtml(program.category || 'DEGREE')}</span></td>
          <td class="p-3">
            <div class="flex flex-col">
              <button type="button" onclick="previewProgram(${program.id})" class="table-title-btn text-left font-semibold text-slate-900 hover:text-amber-700 transition-colors leading-snug text-sm cursor-pointer p-0 bg-transparent border-0 outline-none shadow-none line-clamp-1">${window.AdminUI.escapeHtml(program.title)}</button>
              <span class="text-xs text-slate-400 font-normal line-clamp-1 mt-0.5">${window.AdminUI.escapeHtml(plainExcerpt || '설명 없음')}</span>
            </div>
          </td>
          <td class="p-3 text-center">${getStatusLabel(program.status)}</td>
          <td class="p-3 text-center">
            <button type="button" class="inline-flex items-center gap-1 px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-slate-900 text-xs font-semibold rounded-lg border border-slate-300 shadow-2xs transition-all cursor-pointer" onclick="editProgram(${program.id})" title="프로그램 수정">
              <span>✏️</span> 수정
            </button>
          </td>
        </tr>
  ...
  function initProgramTiptapEditor() {
    ...
    if (!window.HtmlSourceEditor) {
      document.addEventListener('DOMContentLoaded', initProgramTiptapEditor, { once: true });
      return;
    }

    programTiptapInstance = window.HtmlSourceEditor.create('programTiptapEditor', 'programTiptapToolbar', 'programDescription');
  }
  ...
  async function deleteProgram(id) {
    const confirmed = await safeConfirm(`정말 #${id}번 프로그램을 삭제하시겠습니까?`, {
      title: '프로그램 삭제',
      confirmText: '삭제',
      danger: true
    });
    ...
    try {
      await window.AdminApi.api.post('/api/delete-program', formData, { isFormData: true });
    ...
    if (btnBatchDelete) {
      btnBatchDelete.addEventListener('click', async () => {
        ...
        if (!selectedIds.length) {
          window.AdminUI.showToast('삭제할 항목을 먼저 선택해주세요.', 'warning');
          return;
        }

        const confirmed = await safeConfirm(`선택한 ${selectedIds.length}개 프로그램을 영구 삭제하시겠습니까?`, {
          title: '선택 항목 삭제',
          confirmText: '일괄 삭제',
          danger: true
        });
        ...
        try {
          ...
          window.AdminUI.showToast(`${count}개 프로그램이 성공적으로 삭제되었습니다.`, 'success');
          if (selectAllChk) selectAllChk.checked = false;
          loadProgramList();
          if (window.DashboardManager && window.DashboardManager.refresh) {
            window.DashboardManager.refresh();
          ...
          window.AdminUI.showToast(e.message || '프로그램 일괄 삭제 중 오류가 발생했습니다.', 'error');
        }
      });
    }
